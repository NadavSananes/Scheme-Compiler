...
exception X_error_in_the_vector_reader;;
exception X_error_macro_expand_cond_ribs;;
exception X_error_in_the_letrec_expand;;
...
module Reader : READER = struct
  ...
  and nt_vector str =
    ...
    let nt1 = pack (caten nt1 nt2) 
                    (fun (_, e) -> 
                      match e with
                      | ScmNil -> ScmVector ([])
                      | ScmPair (car, cdr) -> ScmVector(pairToList e)
                      | _ -> raise X_error_in_the_vector_reader) in
    nt1 str

  and pairToList pair = 
    match pair with
    | ScmNil -> []
    | ScmPair (car, cdr) -> car :: (pairToList cdr)
    | e -> [e] 
  ...
  and nt_list str =
    ...
    let nt2 = disj nt2 nt3 in
    let nt1 = pack (caten nt1 nt2) (fun (_, e) -> e) in 
    nt1 str
  ...
  and nt_sexpr str = 
    let nt1 =
      disj_list [nt_void; nt_number; nt_boolean; nt_char; nt_symbol;
                 nt_string; nt_list; nt_vector; nt_quoted_forms] in
...
module Tag_Parser : TAG_PARSER = struct
  ...
  | ScmPair (ScmPair (ScmSymbol "unquote-splicing",  
    ...
    | ScmPair (ScmPair (ScmSymbol "unquote-splicing", ScmPair (sexpr, ScmNil)), cdr) -> 
      ScmPair (ScmSymbol "append", ScmPair(sexpr, ScmPair((macro_expand_qq cdr), ScmNil)))
    ...
    | ScmPair(ScmSymbol "unquote-splicing", 
        ScmPair(sexp, ScmNil)) ->
      ScmPair(ScmSymbol "quote", 
      ScmPair(ScmPair(ScmSymbol "unquote-splicing", ScmPair(sexp, ScmNil)), 
      ScmNil))
  ...
  let rec macro_expand_cond_ribs ribs = 
    ...
    | ScmPair (ScmPair( expr, ScmPair(ScmSymbol "=>", ScmPair(exprf, ScmNil))), ScmNil) ->
      ScmPair
      (ScmSymbol "let",
       ScmPair
        (ScmPair
    ...
    | ScmPair (ScmPair(expr, ScmPair(ScmSymbol "=>", ScmPair(exprf, ScmNil))), next_ribs) ->
      ScmPair
      (ScmSymbol "let",
       ScmPair
        (ScmPair
              ...
              (ScmPair
                (ScmSymbol "rest",
                 ScmPair
                  (ScmPair
                    (ScmSymbol "lambda",
                     ScmPair (ScmNil, ScmPair ((macro_expand_cond_ribs next_ribs), ScmNil))),
                   ScmNil)),
               ScmNil))),
         ScmPair
          (ScmPair
    ...
    | ScmPair(
        ScmPair (ScmSymbol "else", expr), ScmNil) ->
      ScmPair (ScmSymbol "begin", expr)

    | ScmPair(
        ScmPair(ScmSymbol "else", expr), next_ribs) ->
      ScmPair (ScmSymbol "begin", expr)
    ...
    | ScmPair(ScmPair(test, dit) , ScmNil) ->
      ScmPair( ScmSymbol "if", 
        ScmPair(test, 
          ScmPair(ScmPair(ScmSymbol "begin", dit), ScmNil)))

    
    | ScmPair (ScmPair(test, dit), next_ribs) ->
      ScmPair( ScmSymbol "if", 
        ScmPair(test, 
          ScmPair(ScmPair(ScmSymbol "begin", dit), 
            ScmPair ((macro_expand_cond_ribs next_ribs), ScmNil))))

    | _ -> raise X_error_macro_expand_cond_ribs;;


  let macro_expand_letrec ribs exprs =  

    let ribs = (match (scheme_list_to_ocaml ribs) with    (* ribs now is a list of the binding pairs *)
                | ribs, ScmNil -> ribs
                | _ -> raise X_error_in_the_letrec_expand) in

    let params =    (*parameter list, variables names and values as 'whatever*)
          scheme_sexpr_list_of_sexpr_list
            (List.map (function
                  | ScmPair (var, ScmPair (_, ScmNil)) -> ScmPair (var, ScmPair
                      (ScmPair (ScmSymbol "quasiquote", ScmPair (ScmSymbol "whatever", ScmNil)), ScmNil))
                  | _ -> raise X_error_in_the_letrec_expand)
            ribs) in

    let setsAll = 
        List.map (function
              | ScmPair (var, ScmPair (arg, ScmNil)) -> ScmPair(ScmSymbol "set!", ScmPair(var, ScmPair(arg, ScmNil)))
              | _ -> raise X_error_in_the_letrec_expand)
        ribs in
    ...
    let exprs = scheme_sexpr_list_of_sexpr_list (setsAll @ exprs) in


    ScmPair (ScmSymbol "let", ScmPair (params, exprs))
...
module Semantic_Analysis : SEMANTIC_ANALYSIS = struct
  ...
  let annotate_lexical_address pe =
    let rec run expr params env =
      ...
      | ScmVarSet( Var (var), expr) -> ScmVarSet'((tag_lexical_address_for_var var params env), 
                                                  (run expr params env))
      | ScmVarDef( Var (var), expr) -> ScmVarDef'((tag_lexical_address_for_var var params env), 
                                                  (run expr params env))
      | ScmLambda(newParams, lambda_kind, expr) -> ScmLambda' (newParams, lambda_kind, 
                                                              match lambda_kind with
                                                              | Simple -> (run expr newParams ([params] @ env))
                                                              | Opt s ->(run expr (newParams @ [s]) ([params] @ env))
                                                              )
      | ScmApplic(expr, exprs) -> ScmApplic'( (run expr params env), 
                                              (List.map (fun expr -> run expr params env) exprs), 
                                              Non_Tail_Call)
    in run pe [] [];;
  ...
  let annotate_tail_calls pe = 
    let rec run expr' in_tc = 
      match expr' with
      | ScmConst'(sexpr) -> ScmConst'(sexpr)
      | ScmVarGet'(var') -> ScmVarGet'(var')
      | ScmIf'(test, dit, dif) -> ScmIf'((run test false), (run dit in_tc), (run dif in_tc))
      | ScmSeq'(exprs') -> ScmSeq'(match exprs' with
                                    | [] -> []
                                    | exprs' ->
                                              (List.map (fun expr' -> run expr' false)
                                              (first_elements exprs')) @ [(run (last_element exprs') in_tc)])

      | ScmOr'(exprs') -> ScmOr'(match exprs' with
                                  | [] -> []
                                  | exprs' ->
                                            (List.map (fun expr' -> run expr' false)
                                            (first_elements exprs')) @ [(run (last_element exprs') in_tc)])
      ...
      | ScmLambda'(params, lambda_kind, expr') -> ScmLambda'(params, lambda_kind, (run expr' true))
      | ScmApplic'(expr', exprs', app_kind) -> ScmApplic'((run expr' false), 
                                                          (List.map (fun expr' -> run expr' false) exprs'), 
                                                          match in_tc with
                                                          | true -> Tail_Call
                                                          | false -> Non_Tail_Call
                                                          )
  ...
  let code_gen exprs' =
    ...
    let rec run params env = function
      ...
      | ScmOr' exprs' ->
         ...
         in asm_code
      | ScmVarSet' (Var' (v, Free), expr') ->
         let exprs_code = run params env expr' in
         let label = search_free_var_table v free_vars in
         let asm_code = 
          exprs_code
      ...
      | ScmVarSet' (Var' (v, Param minor), ScmBox' _) ->
         let v_value_code = run params env (ScmVarGet' (Var' (v, Param minor))) in
         let asm_code = 
         "\tmov rdi, (8 * 1)\t; allocate a box size one word.\n"
         ^ "\tcall malloc\n"
         ^ "\tmov rbx, rax\n"
         ^ v_value_code
         ^ "\tmov [rbx], rax\n"
         ^ (Printf.sprintf "\tmov PARAM(%d), rbx\n" minor)
         ^ "\tmov rax, sob_void\n" 
        in 
        asm_code
      ...
      | ScmBoxSet' (var', expr') ->
         let expr_code = run params env expr' in
         let get_var_code = run params env (ScmVarGet'(var')) in
         let asm_code =
          expr_code
          ^ "\tpush rax\n"
          ^ get_var_code
      ...
      | ScmLambda' (params', Opt opt, body) ->
         ...
         and label_exact_arguments = make_lambda_opt_arity_exact ()
         and label_stack_ok = make_lambda_opt_stack_ok ()
         and label_exact_arguments_loop = make_label_exact_arguments_loop ()
         and label_exact_arguments_loop_exit = make_label_exact_arguments_loop_exit ()
         and label_creat_list_loop = make_label_creat_list_loop ()
         and label_creat_list_loop_exit = make_label_creat_list_loop_exit ()
         and label_more_arguments_loop = make_label_more_arguments_loop ()
         and label_more_arguments_loop_exit = make_label_more_arguments_loop_exit ()
         ...
         ^ (Printf.sprintf "\tcmp qword [rsp + 8 * 2], %d\n"
              (List.length params'))
         ^ (Printf.sprintf "\tjge %s\n" label_arity_ok)  (*We want to jump iff number of given parameters >= params.length*)
         ^ "\tpush qword [rsp + 8 * 2]\n"
         ^ (Printf.sprintf "\tpush %d\n" (List.length params'))
         ^ "\tjmp L_error_incorrect_arity_simple\n"
         ^ (Printf.sprintf "%s:\n" label_arity_ok)

         (* Check if we great than of equal *)
         ^ (Printf.sprintf "\tcmp qword [rsp + 8 * 2], %d\n" (List.length params'))
         ^ (Printf.sprintf "\tje %s\n" label_exact_arguments)

          (* we here iff we have more arguments *)

          (* find the length of the list we need to build. *)
         ^ "\tmov rbx, qword [rsp + 8 * 2]\n"
         ^ (Printf.sprintf "\tsub rbx, %d\t;rbx = size of the new list we building.\n" (List.length params'))
         


         ^ (Printf.sprintf "\tmov rcx, %d\n" (List.length params'))
         ^ "\tadd rcx, 3\t; rcx = The location of the first element in the list.\n"


         (* fix the argument number on the stack *)
         ^ (Printf.sprintf "\tmov dword [rsp + 8 * 2], %d\n" (List.length params' + 1))
         ...
         ^ "\tmov rsi, 0\n"
         ^ (Printf.sprintf "%s:\n" label_creat_list_loop)
         ^ "\tcmp rsi, rbx\n"
         ^ (Printf.sprintf "\tje %s\n" label_creat_list_loop_exit) 
         ^ "\tmov r8, rax\n"
         ^ "\tmov rdx, qword [rsp + 8 * (rcx + 1)]\n"
         ...
         ^ "\tmov SOB_PAIR_CDR(r8), sob_nil\n"
         ^ "\tpop rax\n"
         ^ "\tmov [rsp + 8 * rcx], qword rax\n"
        ...
        (*  rcx hold the current index
            rbx hold the offset to shift each values and the number of pop to do at the end and the stop-index.
            rdi hold the index at offset.
        *)

        ^ (Printf.sprintf "%s:\n" label_more_arguments_loop)

        ^ "\tcmp rdi, 0\n"
        ^ (Printf.sprintf "\tjl %s\n" label_more_arguments_loop_exit)
        ...
        ^ "\tadd rsp, rax\n"
         ...
         ^ (Printf.sprintf "%s:\n" label_exact_arguments_loop)

         ^ "\tcmp rsi, rbx\n"
         ^ (Printf.sprintf "\tje %s\n" label_exact_arguments_loop_exit)
         ^ "\tmov rcx, [rsp + 8 * rdx]\t;rcx = stack[x] \n"
         ^ "\tmov [rsp + 8 * rsi], rcx\t;stack[x-1] = stack[x]\n"
         ^ "\tinc rdx\n"
         ^ "\tinc rsi\n"
         ^ (Printf.sprintf "\tjmp %s\n" label_exact_arguments_loop)
